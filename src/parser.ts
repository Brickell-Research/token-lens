...
export function parse(filePath: string): Node[] {
  const rawEvents = readRawEvents(filePath);
  const tokens = rawEvents
    .map((e) => createToken(e))
    .filter((t) => t.type === "user" || t.type === "assistant");
  const tree = buildTree(tokens, rawEvents);
  attachSubagentTurns(tree, rawEvents);
  attachTaskNotifications(tree);
  return tree;
}

function buildTree(tokens: Token[], rawEvents: RawEvent[]): Node[] {
...
function attachSubagentTurns(tree: Node[], rawEvents: RawEvent[]): void {
  ...
  progressByToolUse.forEach((evts, toolUseId) => {
    ...
    if (!parent) return;
    parent.children.push(...buildSubagentNodes(evts));
  });
}

// Collapse streaming chains (same requestId = one API call) and build tokens.
function buildSubagentNodes(events: RawEvent[]): Node[] {
  ...
  return Array.from(byRequest.values())
    ...
    .map((group) => subagentToken(group))
    .map((t) => ({ token: t, children: [] }));
}

function subagentToken(group: RawEvent[]): Token {
...
function attachTaskNotifications(tree: Node[]): void {
  ...
  all
    ...
    .forEach((node) => {
      ...
      if (!agentNode) return;
      if (!removeNode(tree, node)) return;
...
function removeNode(nodes: Node[], target: Node): boolean {
  const i = nodes.indexOf(target);
  if (i >= 0) {
    nodes.splice(i, 1);
    return true;
  }
  return nodes.some((node) => removeNode(node.children, target));
}

function readRawEvents(filePath: string): RawEvent[] {
  let content: string;
  try {
  ...
  try {
    if (content.trimStart().startsWith("[")) {
      return (JSON.parse(content) as RawEvent[]).map((e) => (e.event ?? e) as RawEvent);
    }
  } catch (e) {
